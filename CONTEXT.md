...
**Windows / Playwright:** The monitor may sit at **0 steps** for a while while workers open the **track menu**. The bridge **retries** opening the track list (`POLYTRACK_TRACK_MENU_ATTEMPTS`, default 4; `POLYTRACK_TRACK_MENU_WAIT_MS` per attempt, default **180s** to allow **.track** HTTP loads), uses a **fallback DOM selector** if `.menu` nesting differs, **staggers** subprocess workers (`POLYTRACK_WORKER_STAGGER_S`), and (for track rows) a **JS probe** that waits out the picker’s **loading** UI until row buttons exist. **Headless on Windows** omits **SwiftShader** by default so GL matches headed (`POLYTRACK_HEADLESS_USE_SWIFTSHADER=1` for GPU-less VMs). **Ctrl+C** or a dead worker often surfaces `TargetClosedError`, `KeyboardInterrupt`, then `BrokenPipeError` / `EOFError` — see **`docs/WINDOWS_TRAINING.md`**; failures are also in **`logs/last_training_error.txt`**.
